...
const Leaderboard = ({ leaderboardType }: UserLeaderboardProps) => {
  ...
  const [leaderboardData, setLeaderboardData] = useState<
    ...
    | {
        data: (UserResponse & { violations: number })[];
  ...
  function fetchLeaderboard() {
    switch (leaderboardType) {
      case LeaderboardType.Kilometers:
        getKilometersLeaderboard(page, order, accessToken).then((users) =>
          setLeaderboardData({
            data: users,
      ...
      case LeaderboardType.Minutes:
        getMinutesLeaderboard(page, order, accessToken).then((users) =>
          setLeaderboardData({
            data: users,
      ...
      case LeaderboardType.Violations:
        getViolationsLeaderboard(page, order, accessToken).then((users) =>
          setLeaderboardData({
            data: users,
            showProperty: "violations",
          })
